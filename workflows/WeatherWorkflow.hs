...
import Control.Exception (IOException, SomeAsyncException, SomeException, catch, displayException, evaluate, fromException, throwIO)
...
import Data.Time (Day, UTCTime (..), defaultTimeLocale, formatTime, fromGregorian, getCurrentTime, parseTimeM)
import Duraflow (ExecutionId (..), RunConfig (..), TaskId (..), Workflow, runWorkflow, task, writeFileDurably)
import GHC.Generics (Generic)
import Network.HTTP.Client (Request (..), defaultRequest, getUri, httpLbs, newManager, parseRequest, responseBody, responseStatus, responseTimeoutNone, setQueryString)
...
data WeatherRequest = WeatherRequest
  { requestedLatitude :: Double, requestedLongitude :: Double, forecastDate :: Day, outputPath :: FilePath }
  deriving (Eq, Show, Generic, ToJSON, FromJSON, NFData)

data ForecastUnits = ForecastUnits
  { timeUnit :: Text, temperatureUnit :: Text, precipitationUnit :: Text, windUnit :: Text, uvUnit :: Text }
  deriving (Eq, Show, Generic, ToJSON, FromJSON, NFData)

data Forecast = Forecast
  { forecastRequest :: WeatherRequest, forecastUnits :: ForecastUnits
  , providerLatitude :: Double, providerLongitude :: Double
  , minimumTemperature :: Double, maximumTemperature :: Double, rainProbability :: Double
  , maximumWindSpeed :: Double, maximumUvIndex :: Double, provider :: Text
  , requestUrl :: Text, retrievedAt :: UTCTime }
  deriving (Eq, Show, Generic, ToJSON, FromJSON, NFData)

data Checklist = Checklist {checklistForecast :: Forecast, checklistItems :: [Text]}
  deriving (Eq, Show, Generic, ToJSON, FromJSON, NFData)
...
weatherPreparation = weatherPreparationWith WeatherEffects {getForecast = fetchForecast, putChecklist = writeChecklist}

forecastUrl :: WeatherRequest -> Text
forecastUrl request = Text.pack (show (getUri configured))
 where
  configured = setQueryString query defaultRequest {secure = True, host = "api.open-meteo.com", port = 443, path = "/v1/forecast"}
  encodedDate = ByteString8.pack (show (forecastDate request))
  query = [("latitude", Just (number (requestedLatitude request))), ("longitude", Just (number (requestedLongitude request))),
    ("start_date", Just encodedDate), ("end_date", Just encodedDate), ("timezone", Just "UTC"),
    ("temperature_unit", Just "celsius"), ("wind_speed_unit", Just "kmh"),
...
parseForecast request url timestamp bytes = firstText (eitherDecodeStrict' bytes) >>= \value ->
...
parseDaily request url timestamp latitude longitude units daily = do
  timeU <- units .: "time"; minU <- units .: "temperature_2m_min"; maxU <- units .: "temperature_2m_max"
  rainU <- units .: "precipitation_probability_max"; windU <- units .: "wind_speed_10m_max"; uvU <- units .: "uv_index_max"
  dates <- daily .: "time"; mins <- daily .: "temperature_2m_min"; maxs <- daily .: "temperature_2m_max"
  rains <- daily .: "precipitation_probability_max"; winds <- daily .: "wind_speed_10m_max"; uvs <- daily .: "uv_index_max"
  either (fail . Text.unpack) pure $ do
    ...
    pure Forecast {forecastRequest = request, forecastUnits = ForecastUnits timeU minU rainU windU uvU,
      providerLatitude = latitude, providerLongitude = longitude, minimumTemperature = at mins,
      maximumTemperature = at maxs, rainProbability = at rains, maximumWindSpeed = at winds,
      maximumUvIndex = at uvs, provider = "Open-Meteo", requestUrl = url, retrievedAt = timestamp}
...
validateCoordinates label latitude longitude = do
  validateFinite (label <> " latitude") latitude; validateFinite (label <> " longitude") longitude
...
fetchForecastWith deadline transport clock request = do
  completed <- timeout deadline $ do
    ...
    validated <- evaluate (force parsed); timestamp <- clock
...
productionTransport url = do
  manager <- newManager tlsManagerSettings; request <- parseRequest (Text.unpack url)
...
prepareAdvice forecast = Checklist forecast (if null advice then ["No additional preparation was identified by these rules."] else advice)
 where advice = [item | (applies, item) <- [(rainProbability forecast >= 50, "Bring rain protection."),
          ...
          (maximumUvIndex forecast >= 3, "Use sun protection.")], applies]

renderChecklist :: Checklist -> Text
renderChecklist Checklist {checklistForecast = forecast, checklistItems} = Text.unlines
  (["Weather preparation checklist", "Forecast date (" <> timeUnit units <> "): " <> showText (forecastDate request),
    ...
    "Provider: " <> provider forecast, "Retrieved at: " <> Text.pack (formatTime defaultTimeLocale "%Y-%m-%dT%H:%M:%S%QZ" (retrievedAt forecast)),
    metric "Minimum temperature" (minimumTemperature forecast) (temperatureUnit units), metric "Maximum temperature" (maximumTemperature forecast) (temperatureUnit units),
    metric "Rain probability" (rainProbability forecast) (precipitationUnit units), metric "Maximum wind speed" (maximumWindSpeed forecast) (windUnit units),
    metric "Maximum UV index" (maximumUvIndex forecast) (uvUnit units), "Preparation:"] <> map ("- " <>) checklistItems)
 where
  request = forecastRequest forecast; units = forecastUnits forecast
...
writeChecklist (target, checklist) = writeFileDurably target (Text.encodeUtf8 (renderChecklist checklist)) >> pure target
...
parseArguments arguments = case arguments of
  [state, execution, latitudeText, longitudeText, dateText, output] -> do
    latitude <- parseCoordinate "latitude" (-90) 90 latitudeText; longitude <- parseCoordinate "longitude" (-180) 180 longitudeText
    day <- parseDate dateText; pure (state, ExecutionId (Text.pack execution), WeatherRequest latitude longitude day output)
...
normalizeInvocation (state, execution, request) = do
  requireDirectory "state directory" state; canonicalState <- canonicalizePath state
  let suppliedOutput = outputPath request; outputName = takeFileName suppliedOutput; suppliedParent = takeDirectory suppliedOutput
  when (null outputName || outputName == "." || outputName == "..") $ ioError (userError "output file must have a filename")
  requireDirectory "output parent directory" suppliedParent; canonicalParent <- canonicalizePath suppliedParent
...
parseCoordinate label lower upper input = case readMaybe input of
  ...
  Just value | isNaN value || isInfinite value -> Left (label <> " must be finite")
...
parseDate input = case parseTimeM True defaultTimeLocale "%F" input of
  ...
  Just day | formatTime defaultTimeLocale "%F" day /= input -> Left "forecast date must use canonical YYYY-MM-DD form"
           | otherwise -> Right day
...
validateOutputEntry path = do
  ...
  case result of Left () -> pure (); Right status -> unless (isRegularFile status) (ioError (userError "output entry is not a regular file"))
 where
...
handleTopLevelErrors action = action `catch` \exception -> case fromException exception :: Maybe SomeAsyncException of
  Just _ -> throwIO exception
  Nothing -> hPutStrLn stderr (displayException (exception :: SomeException)) >> exitFailure
runCli :: IO ()
runCli = handleTopLevelErrors $ do
  ...
  runWorkflow config request weatherPreparation >>= putStrLn
main :: IO ()
main = runCli
